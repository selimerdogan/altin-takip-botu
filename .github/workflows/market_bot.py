...
headers_general = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0 Safari/537.36"}
...
def get_doviz_yahoo():
    ...
    data_d = {}
    data_e = {} # Emtia (Gümüş/Platin vs buraya eklenebilir)
    try:
        ...
        if not df.empty:
            ...
            for k in liste:
                ...
                if pd.notna(val):
                    ...
                    data_d[key] = round(float(val), 4)
    except: pass
    return data_d, data_e
...
def get_crypto_cmc(limit=250):
    ...
    try:
        ...
        for c in r.json()['data']:
            data[f"{c['symbol']}-USD"] = round(float(c['quote']['USD']['price']), 4)
    except: pass
    return data
...
try:
    ...
    if any(len(v) > 0 for k,v in final_paket.items() if isinstance(v, dict)):
        ...
        print(f"🎉 [{saat}] Piyasa verileri kaydedildi.")
    else:
        print("❌ Veri yok!")
        sys.exit(1)
